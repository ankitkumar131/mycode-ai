...
import { PARTNER_SKILL_FILES } from './bundled/partner-skills.js';
...
`
  ...
  // ─── Partner skills (third-party, MIT, attributed) ────────────────────────
  //
  // Generated by scripts/sync-partner-skills.mjs from upstream repositories.
  // Each carries its unmodified source as UPSTREAM-SKILL.md plus a NOTICE with
  // the source URL, commit SHA and licence. See that script's header for why
  // the frontmatter is re-emitted rather than used verbatim.
  ...(['ponytail', 'humanizer', 'strategic-compact'] as const).map(
    (name): BundledSkill => ({ name, files: PARTNER_SKILL_FILES[name] })
  ),
];
